...
class Midtrast {
  static midTransaction = async (data, topup) => {
    // Create Snap API instance
    // console.log();
    let snap = new midtransClient.Snap({
      // Set to true if you want Production Environment (accept real transaction).
  ...
  static createToken = async (snap,parameter,data, topup) => {
    
      let token = await snap(data, topup).createTransaction(
        parameter
      );
      return token;
